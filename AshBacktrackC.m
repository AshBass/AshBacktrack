...
AshBacktrackCInfo threadBacktrackOnlyMainThread(unsigned long frameMaxCount) {
    return  getThreadBacktrackInfo(mainThread, frameMaxCount);
}
...
AshBacktrackCInfo* threadBacktrackWithFrameMaxCount(unsigned long frameMaxCount, int *threadCount) {
    ...
    AshBacktrackCInfo *info = malloc(sizeof(AshBacktrackCInfo)*listCount);
    for (int i = 0; i < listCount; ++i) {
        thread_act_t threadAct = threadList[i];
        info[i] = getThreadBacktrackInfo(threadAct, frameMaxCount);
    }
    *threadCount = listCount;
    return info;
}

AshBacktrackCInfo getThreadBacktrackInfo(thread_act_t threadAct,unsigned long frameMaxCount) {
    
    AshBacktrackCInfo info;
    ...
    unsigned long methodNamesCount = 0;
    info.methodNames = threadStackBacktrack(threadAct, frameMaxCount, &methodNamesCount);
    info.methodNamesCount = methodNamesCount;
    info.isMainThread = (threadAct == mainThread);
    
    return info;
}

char** threadStackBacktrack(thread_act_t thread_act, unsigned long frameMaxCount, unsigned long *methodNamesCount) {
    ...
    char **methodNames = malloc(sizeof(char*)*frameCount);
    for (int i = 0; i < frameCount; ++i) {
        char *methodName = machoImageAddress(buffer[i]);
        methodNames[i] = methodName;
    }
    *methodNamesCount = frameCount;
    return methodNames;
}

char* machoImageAddress(uintptr_t address) {
